...
<body>
    ...
    <table border="1">
        ...
        <tbody>
            ...
            @foreach(Cart::content() as $chitiet)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $chitiet->name }}</td>
                <td>{{ $chitiet->qty }}</td>
                <td style="text-align:right">
                    {{ number_format($chitiet->price) }}<sup><u>đ</u></sup>
                </td>
                <td style="text-align:right">
                    {{ number_format($chitiet->qty * $chitiet->price) }}<sup><u>đ</u></sup>
                </td>
            </tr>
            @php $tongtien += $chitiet->qty * $chitiet->price; @endphp
            @endforeach
            <tr>
